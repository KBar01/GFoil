...
//   - vector_ops.hpp                           — for cnp:: matrix/vector ops
              ...
              int  (&J)[3])
{
    ...
    int il = 0, iu = Ncoords-1, iw = Ncoords;
    ...
    Real (&R_x)[6])
{
    ...
    damp1 = get_damp(th1,ds1,sa1,ue1,param,damp1_U1);
    ...
    if constexpr (ComputeJacobian) {
        ...
        for (int j=0;j<4;++j) {
            ...
            for (int i=0;i<4;++i) { sU1+=cttr_Ut[i]*Ut_U1[i+4*j]; sU2+=cttr_Ut[i]*Ut_U2[i+4*j]; }
            Utt_U1[2+4*j]=sU1; Utt_U2[2+4*j]=sU2;
        }
        Utt_x1[2]=0.0; Utt_x2[2]=0.0;
        for (int i=0;i<4;++i) { Utt_x1[2]+=cttr_Ut[i]*Ut_x1[i]; Utt_x2[2]+=cttr_Ut[i]*Ut_x2[i]; }

        // Split Rl_U / Rt_U into sub-blocks.
        Real Rl_U1[12]={0},Rl_Utl[12]={0},Rt_Utt[12]={0},Rt_U2b[12]={0};
        for (int r=0;r<3;++r)
            for (int c=0;c<4;++c) {
        ...
        cnp::add_inplace<12>(R_U2,Rt_U2b);
        ...
        // R_x (final pass via full chain rule).
    ...
    Real (&R_x)[6])
{
    ...
    // Ut_x1/Ut_x2/Ut_U1/Ut_U2 declared always so cttr chain rule can reference
    // them inside the Jacobian block.
    Real Ut_x1[4]={0}, Ut_x2[4]={0};
    Real Ut_U1[16]={0}, Ut_U2[16]={0};
    if constexpr (ComputeJacobian) {
        for (int i=0;i<4;++i) {
        ...
        for (int i=0;i<4;++i) {
            int idx = colMajorIndex(i,i,4);
            Ut_U1[idx] = w1;
            Ut_U2[idx] = w2;
        }
    }

    // Utl/Utt state values — always needed.
    Real Utl[4]={0}, Utt[4]={0};
    for (int i=0;i<4;++i) { Utl[i]=Ut[i]; Utt[i]=Ut[i]; }
    ...
    // residual_station calls — ComputeJacobian passed through.
    Real Rl[3]={0}, Rl_U[24]={0}, Rl_x[6]={0};
    residual_station<ComputeJacobian>(U1,Utl,x1,xt,Real(0.0),Real(0.0),false,false,false,
                                       param,Rl,Rl_U,Rl_x);
    Rl[2]=0.0;
    if constexpr (ComputeJacobian) {
        for (int i=0;i<8;++i)  Rl_U[colMajorIndex(2,i,3)]=0.0;
        for (int i=0;i<2;++i)  Rl_x[colMajorIndex(2,i,3)]=0.0;
    }

    Real Rt[3]={0}, Rt_U[24]={0}, Rt_x[6]={0};
    residual_station<ComputeJacobian>(Utt,U2,xt,x2,Real(0.0),Real(0.0),false,true,false,
                                       param,Rt,Rt_U,Rt_x);
    for (int i=0;i<3;++i) R[i]=Rl[i]+Rt[i];
    ...
    if constexpr (ComputeJacobian) {
        ...
        Utl_x1[2]=0.0; Utl_x2[2]=0.0;
        for (int j=0;j<4;++j) { Utl_U1[colMajorIndex(2,j,4)]=0.0; Utl_U2[colMajorIndex(2,j,4)]=0.0; }
        ...
        // R_x.
        for (int i=0;i<3;++i) { R_x[i]=Rl_x[i]; R_x[i+3]=Rl_x[i+3]; }
...
{
    Real _dummy_U[24]={0}, _dummy_x[6]={0};
    residual_transition_forced<false>(U1,U2,x1,x2,param,transPos,R,_dummy_U,_dummy_x);
}
