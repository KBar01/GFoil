...
// at each iteration it assembles the global residual + Jacobian (build_glob_RV), tests RMS convergence (resid_rms
// vs param.rtol), solves for the update (solve_glob), applies it with physical under-relaxation (update_state), then relocates the stagnation point and
// marches the transition front (update_transition). On convergence it stores the state + Jacobian for the adjoint pass; on failure it sets failure_mode.
#include <iostream>
#include <cmath>
#include <Eigen/Dense>
#include "real_type.h"
