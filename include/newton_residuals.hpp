#pragma once
// Full-Jacobian residual functions for the Newton loop.
// Each function returns R (residual) AND R_U (dR/dU Jacobian) and/or R_x (dR/dx).
// The AD path uses the R-only versions in residuals.hpp; the Newton path uses these.

#include "real_type.hpp"
#include "data_structs.hpp"
#include "get_funcs.hpp"
#include "vector_ops.hpp"
#include "residuals.hpp"   // for upwind / upwind_half helpers

template<typename Real>
void wake_sys_full(const Vsol<Real>& vsol, const Foil<Real>& foil, const Glob<Real>& glob, const Param<Real>& param,
    Real (&R)[3], Real (&R_U)[36], int (&J)[3]) {

    const Real* U = glob.U.data();
    const int nc = glob.nc;
    int il = 0, iu = nc-1, iw = nc;
    ...
    R[2] = Uw[2] - ctw;
    J[0] = il; J[1] = iu; J[2] = iw;
    R_U[0] = -1.0;
    R_U[colMajorIndex(1,1,3)] = -1.0;
    for (int i = 0; i < 4; ++i) R_U[colMajorIndex(2,i,3)] = -ctw_Ul[i];
    R_U[colMajorIndex(0,4,3)] = -1.0;
    R_U[colMajorIndex(1,5,3)] = -1.0;
    for (int i = 0; i < 4; ++i) R_U[colMajorIndex(2,i+4,3)] = -ctw_Uu[i];
    ...
    Real (&R)[3], Real (&R_U)[24], Real (&R_x)[6])
{
    ...
    Real thlog_U[8] = {0}; thlog_U[0] = -1.0/th1; thlog_U[4] = 1.0/th2;

    Real uelog = std::log(uk2 / uk1);
    Real uelog_U[8] = {0}; uelog_U[3] = -uk1_u/uk1; uelog_U[7] = uk2_u/uk2;
    ...
    Real Hs1 = get_Hs(th1,ds1,sa1,ue1,param,turb,wake,Hs1_U1);
    Real Hs2 = get_Hs(th2,ds2,sa2,ue2,param,turb,wake,Hs2_U2);
    Real Hs_U[8]={0};
    Real Hs = upwind_half(Hs1,Hs1_U1,Hs2,Hs2_U2,Hs_U);

    Real Hslog = std::log(Hs2/Hs1);
    Real Hslog_U[8]={0};
    for (int i=0;i<4;++i) { Hslog_U[i]=-1/Hs1*Hs1_U1[i]; Hslog_U[i+4]=1/Hs2*Hs2_U2[i]; }

    if (simi) {
        thlog=0; Hslog=0; uelog=1; xlog=1; dx=0.5*(x1+x2);
        for (int i=0;i<8;++i) { thlog_U[i]=0; Hslog_U[i]=0; uelog_U[i]=0; }
        xlog_x[0]=0; xlog_x[1]=0; dx_x[0]=0.5; dx_x[1]=0.5;
    }
    ...
    if (turb) {
        ...
        Real de1 = get_de(th1,ds1,ue1,param,de1_U1);
        Real de2 = get_de(th2,ds2,ue2,param,de2_U2);
        Real de  = upwind_half(de1,de1_U1,de2,de2_U2,de_U);

        Real Us1 = get_Us(th1,ds1,sa1,ue1,param,true,wake,Us1_U1);
        Real Us2 = get_Us(th2,ds2,sa2,ue2,param,true,wake,Us2_U2);
        Real Us  = upwind_half(Us1,Us1_U1,Us2,Us2_U2,Us_U);

        Real Hk1 = get_Hk(th1,ds1,ue1,param,Hk1_U1);
        Real Hk2 = get_Hk(th2,ds2,ue2,param,Hk2_U2);
        Real Hk  = upwind(upw,upw_U,Hk1,Hk1_U1,Hk2,Hk2_U2,Hk_U);

        Real Ret1 = get_Ret(th1,ds1,ue1,param,Ret1_U1);
        Real Ret2 = get_Ret(th2,ds2,ue2,param,Ret2_U2);
        Real Ret  = upwind_half(Ret1,Ret1_U1,Ret2,Ret2_U2,Ret_U);

        Real cf1 = get_cf(th1,ds1,sa1,ue1,true,wake,param,cf1_U1);
        Real cf2 = get_cf(th2,ds2,sa2,ue2,true,wake,param,cf2_U2);
        Real cf  = upwind(upw,upw_U,cf1,cf1_U1,cf2,cf2_U2,cf_U);

        Real dsa = 0.5*(ds1+ds2);
        Real dsa_U[8] = {0,0.5,0,0, 0,0.5,0,0};
        Real uq = get_uq(dsa,dsa_U,cf,cf_U,Hk,Hk_U,Ret,Ret_U,wake,param,uq_U);

        Real cteq1_U1[4]={0}, cteq2_U2[4]={0}, cteq_U[8]={0};
        ...
        Real Clag = param.SlagK/param.GB/(1+Us);
        Real Clag_U[8]={0};
        for (int i=0;i<8;++i) Clag_U[i] = -Clag/(1+Us)*Us_U[i];

        Real ald = wake ? Real(param.Dlr) : Real(1.0);
        ...
        if (simi) {
            Rlag = sa2-sa1; Rlag_U[2]=-1; Rlag_U[6]=1;
        } else {
    ...
    Real Ms1_U[4]={0}, Ms2_U[4]={0}, Ms_U[8]={0};
    Real Ms1 = get_Mach2(ue1,param,Ms1_U);
    Real Ms2 = get_Mach2(ue2,param,Ms2_U);
    Real Ms  = upwind_half(Ms1,Ms1_U,Ms2,Ms2_U,Ms_U);

    Real cfxt1_U[4]={0}, cfxt2_U[4]={0}, cfxtm_U[4]={0};
    Real cfxt1_x, cfxt2_x, cfxtm_x;
    Real cfxt1 = get_cfxt(th1,ds1,sa1,ue1,x1,turb,wake,param,cfxt1_U,cfxt1_x);
    Real cfxt2 = get_cfxt(th2,ds2,sa2,ue2,x2,turb,wake,param,cfxt2_U,cfxt2_x);
    // Explicit Real temporaries avoid CoDiPack expression-type deduction failure
    Real th_m=Real(0.5)*(th1+th2), ds_m=Real(0.5)*(ds1+ds2);
    Real sa_m=Real(0.5)*(sa1+sa2), ue_m=Real(0.5)*(ue1+ue2);
    Real x_m=Real(0.5)*(x1+x2);
    Real cfxtm = get_cfxt(th_m,ds_m,sa_m,ue_m,x_m,turb,wake,param,cfxtm_U,cfxtm_x);
    ...
    for (int i=0;i<8;++i)
        Rmom_U[i] = thlog_U[i] + (H_U[i]+Hw_U[i]-Ms_U[i])*uelog + (2+H+Hw-Ms)*uelog_U[i] - 0.5*xlog*cfxt_U[i];
    Rmom_x[0] = -0.5*xlog_x[0]*cfxt - 0.5*xlog*cfxt_x[0];
    Rmom_x[1] = -0.5*xlog_x[1]*cfxt - 0.5*xlog*cfxt_x[1];

    Real cDixt1_U[4]={0}, cDixt2_U[4]={0}, cDixt_U[8]={0};
    Real cDixt1_x, cDixt2_x, cDixt_x[2]={0};
    Real cDixt1 = get_cDixt(th1,ds1,sa1,ue1,turb,wake,x1,param,cDixt1_U,cDixt1_x);
    Real cDixt2 = get_cDixt(th2,ds2,sa2,ue2,turb,wake,x2,param,cDixt2_U,cDixt2_x);
    Real cDixt  = upwind(upw,upw_U,cDixt1,cDixt1_U,cDixt2,cDixt2_U,cDixt_U);
    cDixt_x[0] = (1-upw)*cDixt1_x;
    cDixt_x[1] = upw*cDixt2_x;

    Real cfxtu_U[8]={0}, cfxtu_x[2]={0};
    Real cfxtu = upwind(upw,upw_U,cfxt1,cfxt1_U,cfxt2,cfxt2_U,cfxtu_U);
    cfxtu_x[0] = (1-upw)*cfxt1_x;
    cfxtu_x[1] = upw*cfxt2_x;

    Real Hss1_U[4]={0}, Hss2_U[4]={0}, Hss_U[8]={0};
    Real Hss1 = get_Hss(th1,ds1,ue1,param,Hss1_U);
    Real Hss2 = get_Hss(th2,ds2,ue2,param,Hss2_U);
    Real Hss  = upwind_half(Hss1,Hss1_U,Hss2,Hss2_U,Hss_U);

    Real Rshape = Hslog + (2*Hss/Hs+1-H-Hw)*uelog + xlog*(0.5*cfxtu-cDixt);
    Real Rshape_U[8]={0}, Rshape_x[2]={0};
    for (int i=0;i<8;++i)
        Rshape_U[i] = Hslog_U[i] + (2*Hss_U[i]/Hs - 2*Hss/(Hs*Hs)*Hs_U[i] - H_U[i] - Hw_U[i])*uelog
                    + (2*Hss/Hs+1-H-Hw)*uelog_U[i] + xlog*(0.5*cfxtu_U[i]-cDixt_U[i]);
    ...
    R[0]=Rmom; R[1]=Rshape; R[2]=Rlag;

    int step=0;
    for (int col=0;col<8;++col) { R_U[step]=Rmom_U[col]; step+=3; }
    step=0;
    for (int col=0;col<8;++col) { R_U[step+1]=Rshape_U[col]; step+=3; }
    step=0;
    for (int col=0;col<8;++col) { R_U[step+2]=Rlag_U[col]; step+=3; }
...
template<typename Real>
void residual_station_forced_full(
    const Real* U1, const Real* U2,
    const Real x1, const Real x2,
    const Real aux1, const Real aux2,
    const bool wake, const bool turb, const bool simi,
    const Param<Real>& param, const Real& ncrit,
    Real (&R)[3], Real (&R_U)[24], Real (&R_x)[6])
{
    // Identical to residual_station_full but uses get_damp_forced (for forced-transition BL init).
    Real th1 = U1[0], th2 = U2[0];
    Real ds1 = U1[1]-aux1, ds2 = U2[1]-aux2;
    Real sa1 = U1[2], sa2 = U2[2];
    Real ue1 = U1[3], ue2 = U2[3];
    ...
    Real thlog = std::log(th2/th1);
    Real thlog_U[8]={0}; thlog_U[0]=-1/th1; thlog_U[4]=1/th2;
    ...
    Real Hs1_U1[4]={0}, Hs2_U2[4]={0}, Hs_U[8]={0};
    Real Hs1 = get_Hs(th1,ds1,sa1,ue1,param,turb,wake,Hs1_U1);
    Real Hs2 = get_Hs(th2,ds2,sa2,ue2,param,turb,wake,Hs2_U2);
    Real Hs  = upwind_half(Hs1,Hs1_U1,Hs2,Hs2_U2,Hs_U);
    ...
    Real Hw1_U1[4]={0}, Hw2_U2[4]={0}, Hw_U[8]={0};
    Real Hw1 = get_Hw(th1,aux1,Hw1_U1), Hw2 = get_Hw(th2,aux2,Hw2_U2);
    Real Hw  = upwind_half(Hw1,Hw1_U1,Hw2,Hw2_U2,Hw_U);

    // Forced-transition path always laminar here (no turb branch needed for BL init)
    Real Rlag, Rlag_U[8]={0}, Rlag_x[2]={0};
    {
        Real damp1_U1[4]={0}, damp2_U2[4]={0}, damp_U[8]={0};
    ...
    Real cDixt2 = get_cDixt(th2,ds2,sa2,ue2,turb,wake,x2,param,cDixt2_U,cDixt2_x);
    Real cDixt  = upwind(upw,upw_U,cDixt1,cDixt1_U,cDixt2,cDixt2_U,cDixt_U);
    cDixt_x[0] = (1-upw)*cDixt1_x; cDixt_x[1] = upw*cDixt2_x;

    Real cfxtu_U[8]={0}, cfxtu_x[2]={0};
    Real cfxtu = upwind(upw,upw_U,cfxt1,cfxt1_U,cfxt2,cfxt2_U,cfxtu_U);
    cfxtu_x[0] = (1-upw)*cfxt1_x; cfxtu_x[1] = upw*cfxt2_x;
    ...
    Real (&R)[3], Real (&R_U)[24], Real (&R_x)[6])
{
    ...
    Real th1=U1[0],th2=U2[0],ds1=U1[1],ds2=U2[1],sa1=U1[2],sa2=U2[2],ue1=U1[3],ue2=U2[3];

    Real damp1_U1[4]={0};
    Real damp1 = get_damp(th1,ds1,sa1,ue1,param,damp1_U1);

    Real dampt, dampt_Ut[4]={0}, Rxt, Rxt_xt, dxt;
    Real Ut[4]={0}, Ut_xt[4]={0};
    Real xt = x1 + 0.5*dx;

    for (int iNewton=0; iNewton<nNewton; ++iNewton) {
        Real w2=(xt-x1)/dx, w1=1-w2;
        for (int i=0;i<4;++i) { Ut[i]=w1*U1[i]+w2*U2[i]; Ut_xt[i]=(U2[i]-U1[i])/dx; }
    ...
    Real xt_U1[4]={0}, xt_U2[4]={0};
    for (int i=0;i<4;++i) { xt_U1[i]=xt_U[i]; xt_U2[i]=xt_U[i+4]; }
    for (int i=0;i<4;++i) { Ut_x1[i]+=Ut_xt[i]*xt_x1; Ut_x2[i]+=Ut_xt[i]*xt_x2; }
    ...
    for (int i=0;i<4;++i) {
        ...
        for (int j=0;j<4;++j) {
            Utl_U1[i+4*j]=Ut_U1[i+4*j]; Utl_U2[i+4*j]=Ut_U2[i+4*j];
            Utt_U1[i+4*j]=Ut_U1[i+4*j]; Utt_U2[i+4*j]=Ut_U2[i+4*j];
        }
    }
    Utl[2]=ncrit; Utl_x1[2]=0; Utl_x2[2]=0;
    for (int j=0;j<4;++j) { Utl_U1[2+4*j]=0; Utl_U2[2+4*j]=0; }

    Real cttr_Ut[4]={0};
    Real cttr = get_cttr(Ut[0],Ut[1],Ut[2],Ut[3],true,param,cttr_Ut);
    Utt[2]=cttr;
    for (int j=0;j<4;++j) {
        Real s1=0,s2=0;
        for (int i=0;i<4;++i) { s1+=cttr_Ut[i]*Ut_U1[i+4*j]; s2+=cttr_Ut[i]*Ut_U2[i+4*j]; }
        Utt_U1[2+4*j]=s1; Utt_U2[2+4*j]=s2;
    }
    Utt_x1[2]=0; Utt_x2[2]=0;
    for (int i=0;i<4;++i) { Utt_x1[2]+=cttr_Ut[i]*Ut_x1[i]; Utt_x2[2]+=cttr_Ut[i]*Ut_x2[i]; }

    Real Rl[3]={0},Rl_U[24]={0},Rl_x[6]={0};
    residual_station_full<Real>(U1,Utl,x1,xt,aux1,aux2,false,false,false,param,Rl,Rl_U,Rl_x);
    Real Rt[3]={0},Rt_U[24]={0},Rt_x[6]={0};
    residual_station_full<Real>(Utt,U2,xt,x2,aux1,aux2,false,true,false,param,Rt,Rt_U,Rt_x);

    for (int i=0;i<3;++i) R[i]=Rl[i]+Rt[i];

    // R_x (these override R_x computed below)
    for (int i=0;i<3;++i) {
        R_x[i]   = Rl_x[i] + Rt_x[i]*xt_x1;
        R_x[i+3] = Rl_x[i+3] + Rt_x[i]*xt_x2;
    }

    Real Rl_U1[12]={0},Rl_Utl[12]={0},Rt_Utt[12]={0},Rt_U2[12]={0};
...
template<typename Real>
void residual_transition_forced_full(
    const Real* U1, const Real* U2,
    const Real x1, const Real x2,
    const Param<Real>& param, const Real transPos,
    Real (&R)[3], Real (&R_U)[24], Real (&R_x)[6])
{
    const Real dx = x2-x1;
    Real xt = transPos;
    Real w2 = (xt-x1)/dx, w1 = 1.0-w2;

    Real Ut[4]={0};
    for (int i=0;i<4;++i) Ut[i] = w1*U1[i]+w2*U2[i];
    ...
    Real Ut_U1[16]={0},Ut_U2[16]={0};
    for (int i=0;i<4;++i) {
        Ut_U1[colMajorIndex(i,i,4)] = w1;
        Ut_U2[colMajorIndex(i,i,4)] = w2;
    }
    ...
    for (int j=0;j<4;++j) { Utl_U1[colMajorIndex(2,j,4)]=0; Utl_U2[colMajorIndex(2,j,4)]=0; }
    ...
    Real Rl[3]={0},Rl_U[24]={0},Rl_x[6]={0};
    residual_station_full<Real>(U1,Utl,x1,xt,0,0,false,false,false,param,Rl,Rl_U,Rl_x);
    Rl[2]=0;
    for (int i=0;i<8;++i) Rl_U[colMajorIndex(2,i,3)]=0;
    for (int i=0;i<2;++i) Rl_x[colMajorIndex(2,i,3)]=0;

    Real Rt[3]={0},Rt_U[24]={0},Rt_x[6]={0};
    residual_station_full<Real>(Utt,U2,xt,x2,0,0,false,true,false,param,Rt,Rt_U,Rt_x);
