...
template<typename Real>
void get_nodes(int topFoundIdx,int botFoundIdx, Real x_target, int* topNodeList,int &topNnodes, int* botNodeList,int &botNnodes, const int* turb){
...
template<typename Real>
Real interpolate_cf(const Real* xcoords, const Real* states, const int* nodeIdx, const int nodeN, Real x_target, const int* turb, const Param<Real>&param) {
...
template<typename Real>
void interpolate_at_95_both_surfaces(const Real* xcoords, const Real* states, const Real*Cps, const Oper<Real>&oper, const int* turb, const Param<Real>&param,
