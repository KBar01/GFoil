...
void get_ueinv(const Isolc<Real>& isolc, const Isolv<Real>& isolv, Real* ueinv){
    const int nc = isolc.nc, nw = isolc.nw;
    for (int i=0; i<nc; ++i)  ueinv[i]    = isolv.edgeVelSign[i]*isolc.gammas[i];
    for (int i=0; i<nw; ++i)  ueinv[nc+i] = isolc.uewi[i];
...
template<typename Real>
void get_cp(Post<Real>& post, const Oper<Real>& oper, const Param<Real>& param, const Real* edgeVelocity) {
    const int Nsys = post.nc + post.nw;
    if (oper.Ma > 0.0) {
