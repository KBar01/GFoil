...
void space_wake_nodes(const Real& wakeLength,const Real& firstPanelLength,Real* wakeSpacing,const Foil<Real>&foil,Wake<Real>&wake){
    ...
    wakeSpacing[0] = 0.0 ;
    Real foilEndS = foil.s[foil.nc-1];
    wake.s[0] = foilEndS + 0.0;

    // Compute cumulative sum
    Real term = firstPanelLength;

    for (int i = 0; i < wake.nw-1; ++i) {
...
void build_wake(const Foil<Real>& foil, const Geom<Real>& geom, const Oper<Real>& op, Isolc<Real>& isol, Wake<Real>& wake){
    ...
    Real firstPanelSize = 0.5*(foil.s[1]-foil.s[0] + foil.s[foil.nc-1]-foil.s[foil.nc-2]);
    Real wakeLength = geom.wakelen*geom.chord ;
    std::vector<Real> wakePanelSizes(wake.nw);
    space_wake_nodes(wakeLength,firstPanelSize,wakePanelSizes.data(),foil,wake); //fills wake panel sizes

    // dont need to make xyw or twm use that in wake Struct through modification
    Real midpointTE[2];
    midpointTE[0] = 0.5*(foil.x[colMajorIndex(0,0,2)] + foil.x[colMajorIndex(0,foil.nc-1,2)]);
    midpointTE[1] = 0.5*(foil.x[colMajorIndex(1,0,2)] + foil.x[colMajorIndex(1,foil.nc-1,2)]);
    
    Real normTE[2];
    Real tangTE[2];
    normTE[0] = foil.x[colMajorIndex(0,foil.nc-1,2)] - foil.x[colMajorIndex(0,0,2)];
    normTE[1] = foil.x[colMajorIndex(1,foil.nc-1,2)] - foil.x[colMajorIndex(1,0,2)];
    ...
    for (int i=0; i<wake.nw-1; ++i){
        ...
        inviscid_velocity(foil,isol.gammas.data(),op.Vinf,op.alpha,wake.x[colMajorIndex(0,i,2)],wake.x[colMajorIndex(1,i,2)],v1Store);
        ...
        inviscid_velocity(foil,isol.gammas.data(),op.Vinf,op.alpha,wake.x[colMajorIndex(0,i+1,2)],wake.x[colMajorIndex(1,i+1,2)],v2);
        norm = norm2(v2);
        v2[0] /= norm;
        v2[1] /= norm;
    ...
    v1Store[0]=0.0;
    v1Store[1]=0.0;
    inviscid_velocity(foil,isol.gammas.data(),op.Vinf,op.alpha,wake.x[colMajorIndex(0,wake.nw-1,2)],wake.x[colMajorIndex(1,wake.nw-1,2)],v1Store);
    norm = norm2(v1Store);
    v1[0] = v1Store[0]/norm;
    v1[1] = v1Store[1]/norm;
    wake.t[colMajorIndex(0,wake.nw-1,2)] = v1[0];
    wake.t[colMajorIndex(1,wake.nw-1,2)] = v1[1];

    isol.uewi[wake.nw-1] = (v1Store[0] * v1[0]) + (v1Store[1] * v1[1]);
...
void stagpoint_find(const Isolc<Real>& isolc, Isolv<Real> & isolv, const Foil<Real>& foil,const Wake<Real>& wake) {
    ...
    int j=0;
    for (; j < nc; ++j) { if (isolc.gammas[j] > 0) break; }
    int I[2] = {j-1, j};
    ...
    Real den = G[1]-G[0];
    Real w1 = G[1]/den, w2 = -G[0]/den;

    isolv.stagArcLocation = w1*S[0] + w2*S[1];
    isolv.stagXLocation[0] = foil.x[colMajorIndex(0,j-1,2)]*w1 + foil.x[colMajorIndex(1,j,2)]*w2;
    isolv.stagXLocation[1] = foil.x[colMajorIndex(1,j-1,2)]*w1 + foil.x[colMajorIndex(1,j,2)]*w2;

    Real st_g1 = G[1]*(S[0]-S[1])/(den*den);
    isolv.sstag_g[0]=st_g1; isolv.sstag_g[1]=-st_g1;

    for (int i=j; i<nc; ++i) isolv.edgeVelSign[i] = 1;
    for (int i=0; i<nc; ++i) isolv.distFromStag[i] = std::abs(foil.s[i]-isolv.stagArcLocation);
    for (int i=0; i<nw; ++i) isolv.distFromStag[nc+i] = wake.s[i]-isolv.stagArcLocation;
}
...
void identify_surfaces(const Isolv<Real>& isolv, Vsol<Real>& vsol) {
    ...
    vsol.Is.push_back(range(isolv.stagIndex[1], isolv.nc));
    vsol.Is.push_back(range(isolv.nc, isolv.nc + isolv.nw));
}
...
void set_wake_gap(const Foil<Real>&foil,const Isolv<Real>&isol,Vsol<Real>&vsol){
    ...
    Real xib ;
    for (int i=0; i<nw;++i){
        xib = (isol.distFromStag[nc+i] - isol.distFromStag[nc])/Lw;
        if (xib <= 1.0){
...
void calc_force(const Oper<Real>&op, const Geom<Real>&geom, const Param<Real>&par,const Foil<Real>&foil, const Glob<Real>&glob, Post<Real>& post) {
    ...
    const int nc = foil.nc, nw = glob.nw;
    const int Nsys = nc + nw;

    std::vector<Real> ue(Nsys, 0);
    for (int i=0;i<Nsys;++i) ue[i] = glob.U[colMajorIndex(3,i,4)];
    get_cp(post, op, par, ue.data());
...
void finishdRdU_AD(const Foil<Real>&foil, const Isolc<Real>&isolc, const Isolv<Real>&isolv, Glob<Real>& glob, Vsol<Real>& vsol, const Oper<Real>& oper) {
    ...
    std::vector<Real> ue(Nsys, 0), ds_v(Nsys), tempRHS(Nsys);
    Real uemax = 0.0;
    ...
    get_ueinv(isolc, isolv, ueinv.data());

    // Step 3: Build Residual R
    for (int i=0; i<Nsys; ++i) ds_v[i] = glob.U[colMajorIndex(1,i,4)];
    for (int i=0; i<Nsys; ++i) tempRHS[i] = ds_v[i] * ue[i];

    Real* Rpointer = &glob.R[3*Nsys];
    // ue_m is (Nsys x Nsys), stored column-major; multiply ue_m * tempRHS
    for (int r=0; r<Nsys; ++r) {
        Real s = 0;
        for (int c=0; c<Nsys; ++c) s += vsol.ue_m[colMajorIndex(r,c,Nsys)] * tempRHS[c];
        Rpointer[r] = s;
    }
...
void stagpoint_move_AD(Isolv<Real>& isol,Glob<Real>& glob,const Foil<Real>& foil,const Wake<Real>& wake,Vsol<Real>&vsol,const int (&currStag)[2]) {
    ...
    for (int i=0; i<isol.nc; ++i)
        isol.distFromStag[i] = std::abs(foil.s[i] - isol.stagArcLocation);
    for (int i=0; i<isol.nw; ++i)
        isol.distFromStag[isol.nc+i] = wake.s[i] - isol.stagArcLocation;

    for (int i=0; i<=I[0]; ++i) isol.edgeVelSign[i] = -1;
    for (int i=I[0]+1; i<isol.nc; ++i) isol.edgeVelSign[i] = 1;
